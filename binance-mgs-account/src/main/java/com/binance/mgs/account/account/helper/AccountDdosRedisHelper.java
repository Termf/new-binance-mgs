...
@Slf4j
public class AccountDdosRedisHelper {

    private static String redisHost = ConfigService.getAppConfig().getProperty("account.ddos.redis.host", "");

    private static Integer redisPort = ConfigService.getAppConfig().getIntProperty("account.ddos.redis.port", 6379);

    private static Boolean configEnableFlag = ConfigService.getAppConfig().getBooleanProperty("account.ddos.redis.config.enable", false);
    ...
    public static RedisTemplate<String, Object> getInstance() {
        if (redisTemplate == null && configEnableFlag.booleanValue() ) {
            synchronized (AccountDdosRedisHelper.class) {
